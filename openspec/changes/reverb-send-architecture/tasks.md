...
- [x] 1.4 Replace the dry/wet crossfade `delayStage <: (_ * (1 - reverbMixS), reverbWet * reverbMixS) :> _` with the send-style split `delayStage <: (_, reverbWet * reverbSendS) :> _` so `reverbWet`'s input stays wired to `delayStage`. The arithmetic form `delayStage + reverbWet * reverbSendS` is rejected: in Faust block-diagram algebra it leaves `reverbWet`'s input disconnected and produces a 1→2 effect that processes silence on the wet path at runtime. Verify the `select2(int(reverbOn), delayStage, reverbOut)` bypass still passes the unattenuated dry signal when `reverbOn = 0`. Validate with `faust faust/synth.dsp -lang ocpp` and confirm `getNumInputs() == 0`. Commit: `feat(reverb): replace crossfade with send-style routing`
