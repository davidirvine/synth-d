...
Parameter ranges (formalized in the `wheel-physics` spec): mass `0.05–5` (default `0.1`), spring `1–70` (default `50`), damping ratio ζ `0.05–1` (default `0.3`). The default is a light mass with a strong spring for a snappy return, underdamped at ζ `0.3` for a pronounced overshoot-and-settle; the mass min sits below the default so the wheel can be tuned even faster. The ζ lower bound is `>0` so the system can never be perfectly undamped (perpetual oscillation). The mass floor (`0.05`) is bounded so that even at maximum spring (`70`) the worst-case `dt`-clamped step (ω·dt ≈ 1.9 at the 50 ms cap) stays within the symplectic-Euler stability limit of 2, and per-step value clamping with velocity-zeroing keeps the rare tab-resume frame bounded regardless.
