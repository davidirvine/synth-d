...
- Values > 0 Hz and < 10 Hz: display with one decimal place (e.g., "0.1 Hz", "9.9 Hz")
...
#### Scenario: Frequency knob label below 10 Hz shows one decimal

- **WHEN** an LFO rate knob is set to 0.1 Hz
- **THEN** label reads "0.1 Hz" (not "0 Hz")

#### Scenario: Frequency knob label at 9.9 Hz shows one decimal

- **WHEN** an LFO rate knob is set to 9.9 Hz
- **THEN** label reads "9.9 Hz" (not "10 Hz")
