...
- **THEN** signal flows: oscillator bank (OSC 1 + OSC 2 + OSC 3) → mixer (level-scaled sum + noise) → ladder filter (with ADSR contour + key tracking) → shimmer reverb (bypass when reverbOn = 0) → VCA (ADSR) → feedback delay (bypass when delayOn = 0) → master volume → stereo output
