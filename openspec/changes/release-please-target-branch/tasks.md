...
- [ ] 2.1 Confirm `release-please-config.json` and `.release-please-manifest.json` are unchanged (target-branch belongs in the workflow per design Decision 1)
- [ ] 2.2 Open a PR from `bugfix/release-please-target-branch` to `develop` via `stax ss --yes --no-prompt`
