...
- [x] 3.6 Add an offline Vitest fixture that drives the DSP with an impulse at `cutoffMod = 18000`, `resonance = 0.97` and asserts `|output| ≤ 1.0` at every sample — formally verifies the `ma.tanh` output guard contains any residual instability
...
- [x] 4.3 (future) Add an offline signal-level test: render a loud transient through the filter at near-maximum resonance with a cutoff sweep and assert peak output ≤ 1.0 — this would formally close the post-filter `ma.tanh` stability guard goal
