...
- [ ] 1.1 Create `src/state/synth.svelte.js` exporting a runes `$state` store keyed by every in-scope param (all knob params + all switch params), seeded from the existing factory `DEFAULTS` plus switch defaults; export the param-name list and the audio-param set
