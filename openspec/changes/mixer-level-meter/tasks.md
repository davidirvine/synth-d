...
- [ ] 4.5 Implement clip latch: when peak > 1.0 set a `clipLit` flag; store the `setTimeout` return value in a variable, call `clearTimeout` on any outstanding handle before scheduling a new 1500ms clip-clear callback so that rapid transients do not accumulate overlapping timers
- [ ] 4.6 Stop polling and set all segments dark when `powered` is false; at the top of every rAF callback, if `powered` is false set all segments dark without calling `getPeak()` and return; also cancel the rAF handle in the reactive block that responds to `powered` going false
