...
- [x] 3.1 Define a `DISCRETE_DOMAINS` table in the import module (NOT in `PARAM_SCHEMA`) giving each switch param its valid integer domain, mirroring the synth's authoritative values (sources: `faust/synth.dsp` `nentry` defs and the panel controls). Enumerate exactly these entries: `osc1Wave`/`osc2Wave`/`osc3Wave` → `0..5`; `osc1Range`/`osc2Range`/`osc3Range` → `-2..2`; `osc3LfoMode`, `noiseType`, `drLock`, `modToOsc1`, `modToOsc2`, `modToFilter`, `glideOn`, `delayOn`, `delayModOn`, `reverbOn` → `0..1`. Omit `keyTrack` (it carries `min`/`max` in `PARAM_SCHEMA` and uses those bounds).
