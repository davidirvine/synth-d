...
- **Feedback instability at high shimmer + high decay** → When both `reverbDecay` and `reverbShimmer` approach 1.0, the loop gain can exceed 1.0 and produce an infinite buildup. Mitigation: `(_, -1) : max : (_, 1) : min` (equivalent to `ba.clip(-1, 1)`, unavailable in FAUST 2.85.5) in the feedback path bounds amplitude symmetrically; if the UltraSheer-style intentional runaway is desired, this can be removed in a follow-up.
