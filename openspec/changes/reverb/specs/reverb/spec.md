...
### Requirement: Shimmer reverb DSP stage inserted after master volume

The system SHALL implement a shimmer reverb stage in `faust/synth.dsp` positioned after the VCA (amp envelope) and master volume, as the final stage before the stereo split. The reverb SHALL use `re.mono_freeverb` as the diffusion body with `ef.transpose` (grain 512, crossfade 256, +12 semitones) in the recirculation feedback path. The feedback tap SHALL be scaled by `reverbShimmer` (0–1). The wet/dry blend SHALL be controlled by `reverbMix` (0–1). The tail length SHALL be controlled by `reverbDecay` (0–1) mapped to freeverb's `fb1` parameter. A `reverbOn` parameter (0 or 1, default 0) SHALL bypass the entire reverb stage via `select2` when 0, passing the master output directly to the stereo split unchanged. Default values: `reverbOn` = 0, `reverbMix` = 0.5, `reverbDecay` = 0.5, `reverbShimmer` = 0.

#### Scenario: reverbOn at zero bypasses the reverb

- **WHEN** `reverbOn` is 0
- **THEN** the master output passes directly to the stereo split with no reverb or shimmer processing applied, and the output is identical to the pre-reverb signal chain
...
- **THEN** the reverb tail continues to decay after the VCA closes, producing the characteristic send-effect bloom; the tail length is governed by `reverbDecay`, not the amp release
...
### Requirement: Reverb panel mounted to the right of Amp Env in the filter-output-grid

The system SHALL mount the `Reverb` panel in `App.svelte` inside the `filter-output-grid`, as the rightmost column to the right of `AmpEnv`. The grid SHALL use three columns (`auto auto auto`): Filter (left), AmpEnv/output (centre), Reverb (right), mirroring the audio signal flow.

#### Scenario: Reverb panel is visible in the UI

- **WHEN** the application loads
- **THEN** a panel labelled "reverb" is visible to the right of the amp env/output panel in the synthesizer interface
