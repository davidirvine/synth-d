...
- [ ] 1.5 Implement powered-off behavior: cancel rAF loop and reset LED to dim state when `powered` transitions to `false`; cancel the rAF handle and any pending latch `setTimeout` in the `$effect` cleanup return function
...
- [ ] 5.2b Update `src/components/AmpEnv.test.js` to replace any `ClipLed`-specific assertions with `LevelLed` checks
