...
Defines the testing strategy for the subtractive synthesizer. Pure function modules are unit tested with Vitest for exact value correctness, Svelte components are tested with @testing-library/svelte for structure and behavior, mutation testing with Stryker validates test suite quality on math-critical modules, and Playwright smoke tests verify end-to-end UI interaction in a real browser.
