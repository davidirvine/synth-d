...
Define the continuous-integration checks that run on every pull request to verify code quality and correctness before merge. The pipeline runs automated unit tests, end-to-end tests, lint, and format checks via GitHub Actions, with cached dependencies and build artifacts for fast feedback.
## Requirements
### Requirement: CI uses cached dependencies
The CI pipeline SHALL use `actions/setup-node` with `cache: 'npm'` for npm dependency caching, cache Faust WASM build outputs keyed on the `faust/synth.dsp` hash, and cache Playwright browser binaries keyed on the `node_modules/@playwright/test/package.json` hash to minimise redundant work.
