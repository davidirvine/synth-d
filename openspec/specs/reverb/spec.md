...
Provides a reverb DSP stage with pre-delay, diffusion body, send-style wet/dry parallel mixing, and high-frequency damping control. Positioned after the tape delay stage in the signal chain. The reverb send level, tail decay, pre-delay, and damping are exposed as MIDI-learnable knobs in the Effects panel.
...
The reverb SHALL use a send-style architecture: the dry signal SHALL pass through unattenuated and the wet signal SHALL be added in parallel scaled by `reverbSend`. The output SHALL be `dry + wet * reverbSend`, expressed in Faust as `delayStage <: (_, reverbWet * reverbSendS) :> _`. The arithmetic form `delayStage + reverbWet * reverbSendS` SHALL NOT be used: it leaves `reverbWet`'s input disconnected under Faust block-diagram semantics, so the wet path would process silence at runtime. The dry path SHALL NOT be attenuated as a function of `reverbSend`. The send amount SHALL be controlled by `reverbSend` (0–1, default 0.3).
