...
Provides a small circular LED indicator component that monitors an audio peak level and displays a continuous color map from green (low) through yellow and orange to bright red (clipping). Replaces the binary `ClipLed` component. Used in panel headers to give at-a-glance signal-level feedback at the mixer output and master output stages.
