...
Defines the seam that keeps synth-d structured as an instrument-agnostic **chassis** (audio engine, MIDI, keyboard, patch storage, store machinery, and generic UI) plus a specific **instrument** layer (its parameter set, DSP, and panels), joined by a single explicit contract: the parameter schema. These requirements keep the chassis reusable — one instrument-owned schema module is the sole source of parameter metadata, the store and UI derive from it without duplication, the generic Shell carries no instrument knowledge, and the chassis couples to the instrument only through the schema and the universal engine param contract.
...
- **THEN** none resolve to a Tier-3 instrument module (Oscillator, Mixer, Filter, AmpEnv, Effects, Modulation, Glide, `filterGains`, or the instrument panel layout), with the sole exception of the parameter-schema module
