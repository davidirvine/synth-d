...
Defines the **visual seam** of the synth chassis — the parallel to `chassis-architecture`'s behavioral seam. A synth's visual identity is expressed entirely through a documented, layered set of CSS custom-property design tokens with a single override point (`theme.css`), not through hardcoded constants in components. These requirements keep the chassis re-skinnable: chassis components reference tokens (never hardcoded identity literals), a palette is the single source of identity and cascades to component tokens, setting the palette reskins the whole app, and the default palette renders pixel-identically to the pre-tokenization app.
...
- **THEN** the screenshots match within a small threshold and the existing `vitest`/`stryker`/`playwright` suites pass; the only permitted test edits are mechanical updates to **serialization** assertions that check a style string (e.g. an element's inline `background` or `fill` now reads as a `var(--token, …)` reference) — no behavioral or pixel assertion is weakened or altered
