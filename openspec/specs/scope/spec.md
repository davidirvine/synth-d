...
Provides a real-time oscilloscope display that visualises the synthesizer's audio output as a live waveform. The scope integrates with the audio engine's AnalyserNode, applies the Moog dark aesthetic, and occupies the output placeholder panel slot in the synth UI layout.
...
### Requirement: Scope panel is positioned in the output placeholder slot

The system SHALL render the `Scope` component in the existing placeholder panel under the Output panel (right column, second row) while preserving the existing six-panel structure.

#### Scenario: Layout order

- **WHEN** the UI is rendered
- **THEN** the scope appears in the panel slot below Output and beside the Modulation/Glide row
