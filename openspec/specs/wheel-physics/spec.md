...
Defines the spring-back physics shared by the MOD and PITCH wheels: a mass-bearing damped harmonic oscillator that returns each wheel to its 0.5 rest position on release. Per-wheel physics parameters (mass, spring strength, damping) are editable via knobs in a popup and persist in local storage.
