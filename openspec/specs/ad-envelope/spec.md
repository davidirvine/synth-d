...
Provides one-shot attack-decay envelope generators triggered on gate rising edges. Two independent instances are used: one for filter cutoff modulation, one for VCA gain control. The envelope fires on key press and decays to zero regardless of whether the gate remains high, enabling percussive and legato playing styles.
