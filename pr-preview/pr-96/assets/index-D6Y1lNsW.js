...
`;
		...
		const { promises: fs } = await __vitePreload(async () => {
			const { promises: fs } = await import("./fs-CEwOPGmO.js");
			return { promises: fs };
		}, [], import.meta.url);
		const { pathToFileURL } = await __vitePreload(async () => {
			const { pathToFileURL } = await import("./url-CqPkecXF.js");
...
async function powerOn() {
	...
	node.setOutputParamHandler((path, value) => {
		if (path === "/synth/mixerPeak") mixerPeakValue = value;
		if (path === "/synth/outputPeak") outputPeakValue = value;
	});
...
function Knob($$anchor, $$props) {
	...
	*/
	...
	function tickXY(tick) {
		return polarToXY(START_ANGLE + tick.pos * SWEEP, tick.r ?? 28);
	}
	/** @param {number} pos */
...
var INDEX_KEY = "synth-d:patches";
var SLOT_PREFIX = "synth-d:patch:";
/** @param {string} key @returns {string | null} */
...
function Shell($$anchor, $$props) {
	...
	const versionLabel = `v1.9.0 (96/merge)`;
	const WHEEL_REST = .5;
	const MOD_REST = 0;
	resetParams();
