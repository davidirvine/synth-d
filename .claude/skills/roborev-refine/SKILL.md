...
Or whatever test command the project uses (check CLAUDE.md). If tests fail, fix the regressions
before proceeding.
...
```bash
roborev comment --commenter roborev-refine --job <job_id> -m "$(cat <<'ROBOREV_MSG_EOF'
<summary of changes>
ROBOREV_MSG_EOF
)"
