...
Or whatever test command the project uses (check CLAUDE.md). If tests fail, fix the regressions before proceeding.
...
```bash
roborev comment --commenter roborev-fix --job <job_id> -m "$(cat <<'ROBOREV_MSG_EOF'
<summary of changes>
ROBOREV_MSG_EOF
)"
...
The comment should reference each finding by severity and file, state what was fixed, and note any findings intentionally skipped. Keep it concise (1-3 sentences). Always use a heredoc to prevent shell injection from review-derived content.
...
5. Runs `npx vitest run` to verify
6. Records comments and closes reviews:
   ```bash
   roborev comment --commenter roborev-fix --job 1019 -m "$(cat <<'ROBOREV_MSG_EOF'
   Fixed null check and added error handling
   ROBOREV_MSG_EOF
   )"
   roborev close 1019
   roborev comment --commenter roborev-fix --job 1021 -m "$(cat <<'ROBOREV_MSG_EOF'
...
4. Runs `npx vitest run` to verify
5. Records comment and closes review:
   ```bash
   roborev comment --commenter roborev-fix --job 1019 -m "$(cat <<'ROBOREV_MSG_EOF'
   Fixed null check in src/component.js and error handling in src/utils.js
   ROBOREV_MSG_EOF
   )"
   roborev close 1019
   ```
