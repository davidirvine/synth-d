...
- The feature PR for `feature/$ARGUMENTS` has been merged to `develop`.
- The user has `cd`-ed back to the develop worktree and pulled `develop`.

If you are not sure whether those are true, ask before proceeding.

**Steps:**

1. Confirm the current directory is the develop worktree and on the `develop` branch.
...
3. Review the archive diff with the user. On approval, commit with a message
   like `archive: $ARGUMENTS` and push to `develop`.
